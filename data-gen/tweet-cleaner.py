...
# the following helper function removes all non-ascii text from string s
def removeNonAscii(s): 
    return "".join(i for i in s if ord(i)<128)

def clean_and_print(whole_tweet): 
    tweet = json.loads(whole_tweet.replace('\\n', ' ').replace('\\t', ' '))
    tweet_text = tweet["text"].lower()
    return removeNonAscii(tweet_text).replace('amp;', '').replace('&gt;', '>').replace('&lt;', '<').lstrip()
